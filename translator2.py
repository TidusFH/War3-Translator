...
Version 9.0 - LLM Support & External Configuration
"""
...
try:
    ...
    print(" ⚠️ Warning: config_manager.py not found - using default patterns")
...
try:
    ...
    LLM_AVAILABLE = False
    print(" ⚠️ Warning: llm_translator.py not found - auto-translate disabled")

# --- Configuration ---
# Load from config manager if available, otherwise use defaults
if CONFIG_AVAILABLE:
    UI_FUNCS = config.get_ui_funcs()
    CODE_TOKENS = config.get_code_tokens()
else:
    # Fallback defaults
    UI_FUNCS = [
...
def scan_jass_dependencies(data: bytes) -> Dict[str, List[str]]:
    ...
    CRITICAL_PATTERNS = [
        ...
        # Variable assignments that look like keys
...
def auto_translate_mode(tokens_dir: str):
    """
    Mode 7: Auto-translate extracted files using LLM.
    """
    if not LLM_AVAILABLE:
        print("❌ LLM Translator not available.")
        return

    print("\n" + "="*70)
    print("MODE 7: AUTO-TRANSLATE (LLM)")
    print("="*70)
    
    # Initialize LLM
    translator = LLMTranslator()
    if not translator.client:
        print("❌ Failed to initialize LLM. Check config.ini.")
        return

    src_lang = input("Source language [zh-cn]: ").strip() or "zh-cn"
    dest_lang = input("Destination language [en]: ").strip() or "en"
    
    # Find all _chinese.txt files
    files = list(Path(tokens_dir).glob("*_chinese.txt"))
    if not files:
        print(f"❌ No *_chinese.txt files found in {tokens_dir}")
        return
    ...
    for file_path in files:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8-sig', newline=None) as f:
                content = f.read()
        except Exception as e:
        ...
        for line in lines:
            ...
            if match:
                ...
                parsed_lines.append({'index': idx, 'text': text})
                
                # Heuristic: If text contains Chinese, it needs translation
                # Or if we want to force translate everything
                if contains_chinese(text):
                    to_translate.append(text)
                    indices.append(idx)
            elif line.strip():
                # Keep non-matching lines as is (unlikely in this format)
                pass
        ...
        print(f"   Translating {len(to_translate)} lines...")
        
        # Batch translate
        batch_size = 20
        translated_texts = []
        
        iterator = range(0, len(to_translate), batch_size)
        if TQDM_AVAILABLE:
            iterator = tqdm(iterator, desc="Translating", unit="batch")
            
        for i in iterator:
            batch = to_translate[i:i+batch_size]
            results = translator.translate_batch(batch, src_lang, dest_lang, context="Warcraft III Map Text")
        ...
        for item in parsed_lines:
            ...
            if idx in translation_map:
                # Keep original format: Index. Translation
                new_lines.append(f"{idx}. {translation_map[idx]}")
            else:
                new_lines.append(f"{idx}. {original}")
...
def main():
    ...
    print("=" * 70)
    print(" Warcraft III Map Translation Tool v9.0 - LLM & Config Support")
    print("=" * 70)
    ...
    if LLM_AVAILABLE:
        print(" ✓ llm_translator.py loaded - Mode 7 (Auto-Translate) available")
    else:
        print(" ⚠️ llm_translator.py not found - Mode 7 disabled")
    ...
    while True:
        ...
        if CAMPAIGN_TRANSLATOR_AVAILABLE:
            print(" 5) campaign  - Campaign translation (.w3n)")
        if LLM_AVAILABLE:
            print(" 7) auto      - Auto-translate extracted files (LLM)")
        print(" 6) quit      - Exit")
        print()

        mode = input("Select mode (1-7): ").strip().lower()
        ...
        if mode in ('1', 'extract'):
            ...
            if not LLM_AVAILABLE:
                print("❌ Mode 7 requires llm_translator.py")
                continue
            ...
            auto_translate_mode(tokens_dir)
